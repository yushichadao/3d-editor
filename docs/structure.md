...
| 鼠标相关操作 | `sec-mouse` | 鼠标操作说明（单击/双击/长按/滚轮） |
| 键盘快捷键 | `sec-kbd` | 完整快捷键列表（含滑动条、数字输入框） |

### 2.7 其他 UI 元素（768-871 行）

| 元素 | 行号 | 功能 |
|------|------|------|
| 声明卡片（可折叠） | 822-849 | 功能介绍 / 作者 / AI 辅助 / 最后更新，点击标题栏 max-height 平滑展开收起 |
| 底部视角栏（可折叠） | 851-865 | 左侧"视角 ▼"pill 按钮 + 右侧 7 个视角按钮（透视/顶视/底视/正视/后视/左视/右视），整体 pill 毛玻璃容器，bb-body 用 max-width + opacity 平滑折叠 |
